...
def _map_ats_record_to_raw_job(record: dict) -> dict:
    ...
    fields = require_airtable_record_fields(
        record, ATS_REQUIRED_FIELD_NAMES, table_hint="ATS", known_schema=ATS_KNOWN_FIELD_NAMES
    )
    record_id = str(record["id"])

    company_links = require_airtable_field(
        fields, "Company", record_id=record_id, table_hint="ATS", known_schema=ATS_KNOWN_FIELD_NAMES
    )
    company_name = None
    ...
    return {
        ...
        "x_url": None,
        "non_negotiables": fields.get("Non Negotiables"),
        "nice_to_have": fields.get("Nice-to-have"),
        "projected_salary": fields.get("Projected Salary"),
    }
